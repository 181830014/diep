...
BoostMenu.prototype.boost = function(idx) {
    ...
    if(this.attrLevel[idx] < 10) {
        ...
        let num = 0;
        socket.emit('upgrade', ['upgrade', maintk.armsId, idx]);
...
MiniMap.prototype.animateCoord = function (nx, ny) {
    // stop
    createjs.Tween.get(this.cursor, {override:true}).to({x:4 + nx * this.scaleX, y:4 + ny * this.scaleY}, 10);
}
    ...
    */
    ...
    for(let i = 0; i < 5; i++) {
        ...
        this.slots[i].addEventListener('click', $.proxy(function() {
            socket.emit('upgrade', ['upgrade', maintk.armsId, 7, this.slots[i].armsId]);
            this.slipOut();
        }, this));
